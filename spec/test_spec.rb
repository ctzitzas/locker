...
RSpec.describe Database do
  describe 'Getting names for entries' do
    ...
    it 'should get an array of names from password category' do
      expect(@data.get_entries('passwords')).to eq ['gmail', 'apple', 'github']
    end

    it 'should get an array of names from server category' do
      expect(@data.get_entries('servers')).to eq ['local', 'work']
    end

    it 'should het an array of names from the notes category' do
      expect(@data.get_entries('notes')).to eq ['list of secret recipes', 'secret phone numbers']
    end
  end
  ...
  describe 'Adding entries to categories' do
    before(:each) do
      ...
      @data = Database.new('temp', 'temp', @empty_data)
    end
  
    it 'should add a password to password category' do
      expect(@data.add_password('gmail', 'chris@gmail.com', 'temporary')).to eq (
        [
          {
          'name' => 'gmail',
    ...
    it 'should add a server to server category' do
      expect(@data.add_server('local', 'admin', 'temporary', '192.168.1.12')).to eq (
        [
          'name' => 'local',
          'user' => 'admin',
          'pword' => 'temporary',
    ...
    it 'should add a note to note category' do
      expect(@data.add_note('Secret recipes', 'List of herbs and spices')).to eq (
        [
          'name' => 'Secret recipes',
  ...
  describe 'Editing entries' do
    ...
    it 'should edit password entry for password' do
      expect(@data.edit_entry('passwords', 0, 'pword', 'changed')).to eq 'changed'
    end

    it 'should edit ip_address entry for server' do
      expect(@data.edit_entry('servers', 0, 'ip_address', 'changed')).to eq 'changed'
    end
...
RSpec.describe App do
  ...
  describe 'Generate strong password' do
    ...
    it 'should generate a password longer than 8 character long password' do
...
RSpec.describe Session do
  describe 'Get entry lists' do
    ...
    it 'should return all information from a note entry as an array' do
      expect(@session.get_entries('notes', 0)).to eq ['list of secret recipes', 'Not much here at the moment']
    end
  end
end
